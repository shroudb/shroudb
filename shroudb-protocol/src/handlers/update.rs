use shroudb_core::{CredentialId, Keyspace, KeyspaceType, Metadata, metadata_from_json};
...
pub async fn handle_update(
    ...
    match keyspace.keyspace_type {
        KeyspaceType::ApiKey => {
            ...
            // Check not revoked
            if entry.state == shroudb_core::ApiKeyState::Revoked {
        ...
        KeyspaceType::RefreshToken => {
            ...
            if entry.state == shroudb_core::RefreshTokenState::Revoked {
                return Err(CommandError::Denied {
                    reason: "credential is revoked".into(),
                });
            }
