...
use shroudb_core::{KeyId, Keyspace, KeyspacePolicy, SigningKeyState};
use shroudb_storage::wal::{SigningKeyAlgorithm, SigningKeyCreatedPayload};
...
pub async fn handle_rotate(
    ...
    match &keyspace.policy {
        ...
        KeyspacePolicy::Hmac {
            ...
            let (_, raw_bytes) = shroudb_crypto::generate_api_key(None)?;
            let new_kid = KeyId::new();
            let version = all_keys_len as u32 + 1;
            let new_state = if staged.is_none() {
                SigningKeyState::Active
            ...
            if let Some(ring) = engine.index().hmac_rings.get(ks_name)
                && let Some(mut key) = ring.find_by_kid(&new_kid)
            {
                key.key_material = Some(shroudb_crypto::SecretBytes::new(raw_bytes.to_vec()));
                ring.replace(key);
            }
