...
use keyva_protocol::{Command, CommandDispatcher};
use keyva_storage::StorageEngine;
use keyva_storage::snapshot::SnapshotReader;
use metrics::gauge;
...
async fn metrics_reporter(engine: Arc<StorageEngine>, mut shutdown_rx: watch::Receiver<bool>) {
    ...
    loop {
        ...
        gauge!("keyva_wal_entries_since_snapshot")
            .set(engine.total_entries_since_snapshot().await as f64);
        gauge!("keyva_uptime_seconds").set(start_time.elapsed().as_secs_f64());

        for entry in engine.index().keyspaces.iter() {
            let name = entry.key();
            // API key counts
            if let Some(idx) = engine.index().api_keys.get(name) {
                gauge!("keyva_credentials_total", "keyspace" => name.clone(), "type" => "api_key")
                    .set(idx.len() as f64);
            }
            // Refresh token counts
            if let Some(idx) = engine.index().refresh_tokens.get(name) {
                gauge!("keyva_credentials_total", "keyspace" => name.clone(), "type" => "refresh_token")
                    .set(idx.len() as f64);
            }
            // Signing key counts
            if let Some(ring) = engine.index().jwt_rings.get(name) {
                gauge!("keyva_signing_keys_total", "keyspace" => name.clone())
                    .set(ring.len() as f64);
            }
            ...
            if let Some(idx) = engine.index().passwords.get(name) {
                gauge!("keyva_credentials_total", "keyspace" => name.clone(), "type" => "password")
                    .set(idx.len() as f64);
            }
            // Revocation set size
            if let Some(revs) = engine.index().revocations.get(name) {
                gauge!("keyva_revocations_active", "keyspace" => name.clone())
                    .set(revs.len() as f64);
            }
                ...
                && let Some(active) = ring.active_key()
            {
                ...
                if let Some(ks) = engine.index().keyspaces.get(name)
                    && let keyva_core::KeyspacePolicy::Hmac { rotation_days, .. } = &ks.policy
                {
                    let rotation_at = active.created_at + (*rotation_days as u64 * 86400);
                    let countdown = rotation_at.saturating_sub(now);
                    gauge!("keyva_rotation_countdown_seconds", "keyspace" => name.clone())
        ...
        {
            gauge!("keyva_snapshot_size_bytes").set(metadata.len() as f64);
        }
    }
}
