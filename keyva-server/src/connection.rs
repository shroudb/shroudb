...
use keyva_protocol::resp3::Resp3Frame;
use keyva_protocol::resp3::parse_command::parse_command;
use keyva_protocol::resp3::reader::read_frame;
use keyva_protocol::resp3::serialize::response_to_frame;
use keyva_protocol::resp3::writer::write_frame;
use keyva_protocol::response::{ResponseMap, ResponseValue};
use keyva_protocol::{CommandDispatcher, CommandResponse};
use metrics::gauge;
...
impl Drop for ConnectionGuard {
    fn drop(&mut self) {
        gauge!("keyva_concurrent_connections").decrement(1.0);
    }
}
...
pub async fn handle_connection(
    ...
    gauge!("keyva_concurrent_connections").increment(1.0);
    ...
    loop {
        ...
        let frame = match frame {
            ...
            Err(e) => {
                ...
                let err_frame =
                    keyva_protocol::Resp3Frame::SimpleError(format!("ERR protocol: {e}"));
        ...
        let command = match parse_command(frame) {
            Ok(cmd) => cmd,
            Err(e) => {
                let err_frame = keyva_protocol::Resp3Frame::SimpleError(format!("ERR {e}"));
        ...
        if let Some(ref mut limiter) = rate_limiter
            && !limiter.try_acquire()
        {
            let response = CommandResponse::Error(keyva_protocol::CommandError::Denied {
        ...
        if let keyva_protocol::Command::Auth { ref token } = command {
        ...
        if let keyva_protocol::Command::Subscribe { ref channel } = command {
