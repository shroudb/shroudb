...
use keyva_crypto::{HmacAlgorithm, JwtAlgorithm};
use keyva_protocol::auth::{AuthPolicy, AuthRegistry};
...
impl Default for ServerConfig {
    fn default() -> Self {
        Self {
            ...
            unix_socket: None,
            tls_client_ca: None,
            rest_bind: Some(default_rest_bind()),
            grpc_bind: None,
            rate_limit: None,
        }
    }
...
fn default_rest_bind() -> SocketAddr {
...
pub fn load(path: &Path) -> anyhow::Result<Option<KeyvaConfig>> {
    if !path.exists() {
        return Ok(None);
    }
    let contents =
...
pub fn to_engine_config(config: &KeyvaConfig) -> StorageEngineConfig {
            ...
            (
                ...
                KeyspacePolicy::Jwt {
                    ...
                    required_claims: ks_config
                        .required_claims
                        .as_ref()
                        .map(|v| keyva_core::metadata_from_json(v.clone()))
                        .transpose()
...
) -> Vec<keyva_protocol::webhooks::WebhookConfig> {
    configs
        .as_ref()
        .map(|cfgs| {
            cfgs.iter()
                .map(|c| keyva_protocol::webhooks::WebhookConfig {
                    url: c.url.clone(),
...
type = "jwt"
"#;
        let cfg: KeyvaConfig = toml::from_str(toml_str).unwrap();
        assert_eq!(cfg.server.bind, default_bind());
