...
#[derive(Parser)]
#[command(name = "keyva", about = "Credential management server", version)]
struct Cli {
...
async fn main() -> anyhow::Result<()> {
    ...
    let cfg = match config::load(&cli.config)? {
        Some(cfg) => {
            ...
            // a separate audit log file routed via the "keyva::audit" target.
            ...
            let audit_filter = tracing_subscriber::filter::Targets::new()
                .with_target("keyva::audit", tracing::Level::INFO);
    ...
    if !webhook_configs.is_empty() {
        let (dispatcher_wh, rx) = keyva_protocol::WebhookDispatcher::new(webhook_configs.clone());
        tracing::info!(
            count = webhook_configs.len(),
            "webhook dispatcher initialized"
        );
        // Spawn the background delivery loop.
        tokio::spawn(keyva_protocol::webhooks::webhook_delivery_loop(
            rx,
            webhook_configs,
        ));
    ...
    tracing::info!(bind = %cfg.server.bind, "keyva ready");
    ...
    // 15. Clean exit.
    tracing::info!("keyva shut down cleanly");
    Ok(())
}
...
fn resolve_master_key() -> anyhow::Result<Box<dyn MasterKeySource>> {
    // Check if KEYVA_MASTER_KEY or KEYVA_MASTER_KEY_FILE is set.
...
async fn handle_rekey(
    ...
    let old_source = FixedKeySource(keyva_crypto::SecretBytes::new(old_key_bytes));
    ...
    let new_source = FixedKeySource(keyva_crypto::SecretBytes::new(new_key_bytes));
    let new_km = keyva_storage::key_manager::KeyManager::new(
        &new_source,
        keyva_core::TenantContext::default(),
    )
    .await?;
    ...
    for entry in index.keyspaces.iter() {
        ...
        if let Some(ring) = index.jwt_rings.get(&name) {
            for key in ring.all_keys() {
                if let Some(ref pk) = key.private_key {
                    let encrypted = keyva_crypto::aes_gcm_encrypt(
                        new_km.private_key_key(&name)?.as_bytes(),
                        pk.as_bytes(),
                        b"private_key",
                    )?;
                    encrypted_private_keys.push(
                        keyva_storage::snapshot::format::EncryptedPrivateKey {
        ...
        keyspaces.push(keyva_storage::snapshot::format::KeyspaceSnapshot {
            keyspace: ks_snap,
            encrypted_private_keys,
        });
    }

    let snapshot_data = keyva_storage::snapshot::format::SnapshotData {
        keyspaces,
        revocations: Vec::new(),
    };
    ...
    let header = keyva_storage::snapshot::format::SnapshotHeader {
        version: 1,
        encoding: "postcard-v1".to_string(),
        created_at: now,
        snapshot_id: uuid::Uuid::new_v4().to_string(),
        namespace: namespace.as_str().to_string(),
        wal_checkpoint: keyva_storage::wal::writer::WalCheckpoint {
            segment_seq: u64::MAX,
            byte_offset: 0,
            entry_count: 0,
        },
    ...
    let writer =
        keyva_storage::snapshot::writer::SnapshotWriter::new(data_dir.clone(), namespace.clone());
    writer
        .write(
            &header,
            &snapshot_data,
    ...
    let wal_reader =
        keyva_storage::wal::reader::WalReader::new(data_dir.clone(), namespace.clone());
    ...
    println!("Rekey complete. Update KEYVA_MASTER_KEY to the new key before restarting.");
    Ok(())
}

/// A fixed master key source for rekey/export/import operations.
struct FixedKeySource(keyva_crypto::SecretBytes);
...
async fn handle_export(config_path: &Path, keyspace: &str, output: &Path) -> anyhow::Result<()> {
    ...
    let export_enc_key = keyva_crypto::derive_key(
        resolve_master_key()?.load().await?.as_bytes(),
        "default",
        "__export__",
        32,
    )?;
    let export_hmac_key = keyva_crypto::derive_key(
        resolve_master_key()?.load().await?.as_bytes(),
        "default",
        "__export_hmac__",
        32,
    ...
    if let Some(ring) = index.jwt_rings.get(keyspace) {
        for key in ring.all_keys() {
            if let Some(ref pk) = key.private_key {
                ...
                encrypted_private_keys.push(keyva_storage::snapshot::format::EncryptedPrivateKey {
                    key_id: key.key_id.clone(),
                    encrypted_bytes: encrypted,
                });
            }
    ...
    let export_snap = keyva_storage::snapshot::format::KeyspaceSnapshot {
        keyspace: ks_export,
        encrypted_private_keys,
    };
    ...
    let encrypted_payload =
        keyva_crypto::aes_gcm_encrypt(export_enc_key.as_bytes(), &payload_bytes, b"export")?;
    ...
    let hmac = keyva_crypto::hmac_sign(
...
async fn handle_import(
    ...
    if &data[..4] != EXPORT_MAGIC {
        anyhow::bail!("not a valid keyva export bundle (bad magic)");
    }
    ...
    let export_enc_key =
        keyva_crypto::derive_key(master_key.as_bytes(), "default", "__export__", 32)?;
    let export_hmac_key =
        keyva_crypto::derive_key(master_key.as_bytes(), "default", "__export_hmac__", 32)?;
    ...
    let plaintext =
        keyva_crypto::aes_gcm_decrypt(export_enc_key.as_bytes(), encrypted_payload, b"export")?;

    let ks_snap: keyva_storage::snapshot::format::KeyspaceSnapshot =
    ...
    for key in &ks_snap.keyspace.signing_keys {
        ...
        if let Some(epk) = ks_snap
            ...
            && let Ok(decrypted) = keyva_crypto::aes_gcm_decrypt(
                export_enc_key.as_bytes(),
                &epk.encrypted_bytes,
                b"export_pk",
            )
        {
            key.private_key = Some(keyva_crypto::SecretBytes::new(decrypted));
        }
    ...
    for key in &ks_snap.keyspace.hmac_keys {
        ...
        {
            key.key_material = Some(keyva_crypto::SecretBytes::new(decrypted));
        }
...
async fn handle_doctor(config_path: &Path) -> anyhow::Result<()> {
    ...
    // 2. Master key check
    let key_source_name = if std::env::var("KEYVA_MASTER_KEY").is_ok() {
        Some("KEYVA_MASTER_KEY")
    } else if std::env::var("KEYVA_MASTER_KEY_FILE").is_ok() {
        Some("KEYVA_MASTER_KEY_FILE")
    } else {
        None
    };
    ...
    let engine_config = config::to_engine_config(&cfg);
    let namespace = engine_config.namespace.clone();
    let wal_reader = keyva_storage::wal::reader::WalReader::new(
    ...
    match wal_reader.list_segments().await {
        ...
        Ok(segments) => {
            ...
            for (_seq, path) in &segments {
                match wal_reader
                    .read_segment(path, keyva_storage::RecoveryMode::Recover)
                    .await
                {
                    Ok(result) => {
    ...
    // 5. Snapshot check
    let snap_reader = keyva_storage::snapshot::reader::SnapshotReader::new(
        engine_config.data_dir.clone(),
        namespace,
    );
            ...
            if let Some(ref ks) = key_source {
                match ks.load().await {
                    Ok(_master_key) => {
                        ...
                        match keyva_storage::key_manager::KeyManager::new(&**ks, tc).await {
                            Ok(km) => {
                                match snap_reader
                                    .load(
                                        &snap_path,
