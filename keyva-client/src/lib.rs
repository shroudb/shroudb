//! `keyva-client` — typed Rust client library for Keyva.
//!
//! Provides a high-level async API for interacting with a Keyva server over TCP.
...
//!
//! # async fn example() -> Result<(), keyva_client::ClientError> {
//! let mut client = KeyvaClient::connect("127.0.0.1:6399").await?;
//!
//! // Issue an API key
...
/// Parsed components of a Keyva connection URI.
...
///
/// ```
/// use keyva_client::parse_uri;
///
/// let cfg = parse_uri("keyva://localhost").unwrap();
/// assert_eq!(cfg.host, "localhost");
/// assert_eq!(cfg.port, 6399);
/// assert!(!cfg.tls);
///
/// let cfg = parse_uri("keyva+tls://mytoken@prod.example.com:7000/sessions").unwrap();
/// assert!(cfg.tls);
...
pub fn parse_uri(uri: &str) -> Result<ConnectionConfig, ClientError> {
    let (tls, rest) = if let Some(rest) = uri.strip_prefix("keyva+tls://") {
        (true, rest)
    } else if let Some(rest) = uri.strip_prefix("keyva://") {
        (false, rest)
    } else {
...
/// A client for interacting with a Keyva server.
pub struct KeyvaClient {
    pub(crate) connection: Connection,
}

impl KeyvaClient {
    /// Connect to a Keyva server at the given address (e.g. `"127.0.0.1:6399"`).
    pub async fn connect(addr: &str) -> Result<Self, ClientError> {
        let connection = Connection::connect(addr).await?;
        Ok(Self { connection })
    }

    /// Connect to a Keyva server over TLS at the given address (e.g. `"127.0.0.1:6399"`).
    ///
...
mod tests {
    ...
    fn parse_uri_with_port() {
        let cfg = parse_uri("keyva://localhost:7000").unwrap();
        assert_eq!(cfg.host, "localhost");
        assert_eq!(cfg.port, 7000);
    }
    ...
    #[test]
    fn parse_uri_tls() {
        let cfg = parse_uri("keyva+tls://prod.example.com").unwrap();
        assert!(cfg.tls);
        assert_eq!(cfg.host, "prod.example.com");
        assert_eq!(cfg.port, 6399);
    }
    ...
    fn parse_uri_with_auth() {
        let cfg = parse_uri("keyva://mytoken@localhost:6399").unwrap();
        assert_eq!(cfg.auth_token.as_deref(), Some("mytoken"));
        assert_eq!(cfg.host, "localhost");
        assert_eq!(cfg.port, 6399);
    }
    ...
    fn parse_uri_with_keyspace() {
        let cfg = parse_uri("keyva://localhost/sessions").unwrap();
        assert_eq!(cfg.keyspace.as_deref(), Some("sessions"));
        assert_eq!(cfg.host, "localhost");
        assert_eq!(cfg.port, 6399);
    }
    ...
    fn parse_uri_full_form() {
        let cfg = parse_uri("keyva+tls://tok@host:7000/keys").unwrap();
        assert!(cfg.tls);
    ...
    fn parse_uri_default_port_on_invalid_port() {
        let cfg = parse_uri("keyva://localhost:notaport").unwrap();
        assert_eq!(cfg.port, 6399);
    }
}
