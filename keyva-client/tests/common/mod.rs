...
/// A test server that spawns the actual `keyva` binary on a random port.
...
"#,
        ...
        // Find the keyva binary in the workspace target directory.
        ...
        let binary = find_keyva_binary();

        let child = StdCommand::new(&binary)
            .arg("--config")
            .arg(&config_path)
            .env("KEYVA_MASTER_KEY", master_key)
            .env("RUST_LOG", "warn")
            .spawn()
            .unwrap_or_else(|e| panic!("failed to start keyva at {}: {e}", binary.display()));
        ...
        if !connected {
            panic!("keyva server did not start within 5 seconds on {addr}");
        }
...
/// Locate the `keyva` binary in the workspace target directory.
fn find_keyva_binary() -> std::path::PathBuf {
    ...
    panic!(
        "Could not find keyva binary. Build it first with `cargo build -p keyva`. \
         Searched at: {}",
        path.display()
    );
}
