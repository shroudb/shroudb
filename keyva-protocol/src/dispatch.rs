...
impl CommandDispatcher {
    ...
    pub async fn execute(&self, cmd: Command, auth: Option<&AuthPolicy>) -> CommandResponse {
        ...
        counter!("keyva_commands_total", "command" => verb, "keyspace" => keyspace_label.clone(), "result" => result_label).increment(1);
        histogram!("keyva_command_duration_seconds", "command" => verb, "keyspace" => keyspace_label.clone()).record(duration.as_secs_f64());

        // Phase 0 replication metric: command behavior classification
        counter!("keyva_commands_by_behavior_total", "behavior" => behavior).increment(1);

        // Phase 0 replication metric: VERIFY rate histogram
        if is_verify {
            histogram!("keyva_verify_rate", "keyspace" => keyspace_label.clone())
    ...
    async fn dispatch(
        &self,
        cmd: Command,
        keyspace: Option<&keyva_core::Keyspace>,
