...
pub async fn handle_password_set(
    ...
    let hash = keyva_crypto::password_hash(
        plaintext.as_bytes(),
        hash_params.m_cost,
        hash_params.t_cost,
        hash_params.p_cost,
...
pub async fn handle_password_verify(
    ...
    {
        counter!("keyva_password_lockout_total", "keyspace" => ks_name.clone()).increment(1);
    ...
    let valid = keyva_crypto::password_verify(plaintext.as_bytes(), &entry.hash)?;

    if !valid {
        counter!("keyva_password_verify_failed_total", "keyspace" => ks_name.clone()).increment(1);
    ...
    let needs_rehash = keyva_crypto::password_needs_rehash(
        &entry.hash,
        hash_params.m_cost,
        hash_params.t_cost,
        hash_params.p_cost,
    ...
    if needs_rehash {
        counter!("keyva_password_rehash_total", "keyspace" => ks_name.clone()).increment(1);
        // Fire-and-forget rehash: hash with new params and write WAL
        if let Ok(new_hash) = keyva_crypto::password_hash(
            plaintext.as_bytes(),
            hash_params.m_cost,
            hash_params.t_cost,
            hash_params.p_cost,
...
pub async fn handle_password_change(
    ...
    // Verify old password
    let valid = keyva_crypto::password_verify(old_plaintext.as_bytes(), &entry.hash)?;
    if !valid {
        return Err(CommandError::Denied {
    ...
    // Hash new password
    let new_hash = keyva_crypto::password_hash(
...
pub async fn handle_password_import(
    ...
    keyva_crypto::validate_imported_hash(hash_bytes)?;

    // Detect algorithm from the hash
    let algorithm = match keyva_crypto::detect_hash_algorithm(hash_bytes) {
