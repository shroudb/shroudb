...
pub async fn handle_issue(
    ...
    match &keyspace.policy {
        KeyspacePolicy::Jwt {
            ...
            let token = keyva_crypto::sign_jwt(private_key.as_bytes(), *algorithm, &claims, kid)?;
        ...
        KeyspacePolicy::ApiKey { prefix } => {
            let (api_key, _raw_bytes) = keyva_crypto::generate_api_key(prefix.as_deref())?;
            let key_hash = keyva_crypto::sha256(api_key.as_bytes());
            ...
            let entry = ApiKeyEntry {
                ...
                key_hash,
                prefix: prefix.clone(),
                state: keyva_core::ApiKeyState::Active,
                metadata: meta,
                created_at: now,
                expires_at,
        ...
        KeyspacePolicy::Hmac { algorithm, .. } => {
            ...
            let signature =
                keyva_crypto::hmac_sign(*algorithm, key_material.as_bytes(), &payload_bytes)?;
        ...
        KeyspacePolicy::RefreshToken { token_ttl_secs, .. } => {
            let (token, _raw_bytes) = keyva_crypto::generate_api_key(None)?;
